...
def get_current_delegation_ops():
    op_list = []
    delegators = []
    for op in sfr.history_reverse(only_ops=["delegate_vesting_shares"]):
...
def get_distributions(payout_posts): # Obtains payouts from the delimited post list
    ...
    for flagger in flaggers:
        ...
        if cursor.execute("SELECT count(paid == 1) FROM steemflagrewards WHERE flagger == '{}'".format(flagger)).fetchone()[0] == 0:
            new_flag = cursor.execute("SELECT flagger, flag_rshares, post FROM steemflagrewards WHERE post IN ({}) " \
        ...
        first_flags = cursor.execute("SELECT flagger, flag_rshares, post FROM steemflagrewards WHERE post IN ({}) " \
        ...
        for firstflag in first_flags:
            ...
            dist_list.append(dist_dict)
        follow_on = cursor.execute("SELECT flagger, flag_rshares, post FROM steemflagrewards WHERE post IN ({}) " \
...
def get_payout_flags(): #Pulls votes from all flag mentions on posts older than 7 days and registers in db.
    payout_posts = []
    sql = cursor.execute("SELECT DISTINCT post, flagger, flag_rshares, approved_by, category, comment, resolved FROM steemflagrewards WHERE parent_created < DATETIME(\'now\', \'-7 days\') AND paid == 0 GROUP BY post")
        ...
        for v in p['active_votes']:
            ...
            if v['rshares'] < 0 and IsRegistered(v['voter']): #gets other flags on the post for registered abuse fighters
                ...
                if cursor.execute("SELECT * FROM steemflagrewards WHERE post == '{}' AND flagger == '{}';".format((q[0]),(flagger))).fetchone(): #checks if flag exists in db
                    continue
                #insert_mention(approving_mod_steem_acct,cats,dust,flagger, p, v, weight, paid,follow_on)
...
def get_daily_distribution(payout_posts,dist_list): #Executes payments against SFR daily rewards pool. Remainder given to delegators
    ...
    payout_dict = {}
    sfr.refresh()
    sfr_daily_sbd = (stm.sp_to_sbd(sfr.sp)*10)*2 #calculated vote amount of SFR account in SBD
    fund_exhausted = False
    sfr_vote_sbd = stm.sp_to_sbd(sfr.sp)
    #sfr_vote_sbd = 13
    dops = get_current_delegation_ops()
    delegators = get_payout_delegators(payout_posts,dops)
    ...
    if sfr_daily_sbd > 0:
        del_pct = get_current_delegators_pct(dops)
        for delegator in del_pct:
...
def get_payout_delegators(payout_posts,ops):
    delegator_list = []
    delegator_dict = {}
    flaggers = cursor.execute("SELECT DISTINCT flagger FROM steemflagrewards WHERE post IN ({})".format(','.join(payout_posts))).fetchall()
    for flagger in flaggers:
        flagger = flagger.__getitem__(0)
        SP = delegation_check(flagger,ops)
        if SP == None:
            continue
        delegator_dict = {'flagger': flagger, 'SP': round(SP,3)}
        delegator_list.append(delegator_dict)
...
def token_report(payout_list):
    ...
    rep = stm.post(
        ...
        token_report, cfg.SFRTACCOUNT, tags=['steemflagrewards', 'abuse', 'steem', 'steem-engine','busy'],
        parse_body=True, self_vote=False, community='busy', app='busy/2.5.6')
    return construct_authorperm(rep['operations'][0][1])
...
async def approve(ctx, link):
    ...
    last_tkn_post_age = (Comment(construct_authorperm(sfrt.get_blog_entries()[0]))).time_elapsed()
    if last_tkn_post_age > datetime.timedelta(hours=24):
        ...
        dist = get_distributions(payout_flags)
        await ctx.send("Obtained distribution list")
        cred = get_daily_distribution(payout_flags,dist)
